(() => {
  ...
  function visibleSelectableRows() {
    ...
    return [...body.querySelectorAll('tr')].filter(row => {
      ...
      if (!cb) return false;
      if (row.classList.contains('shipped-hidden')) return false;
  ...
  function updateBulkState() {
    ...
    if (count) count.textContent = `已選 ${ids.length} 筆`;
  ...
  function ensureBulkBar() {
    ...
    bar.innerHTML = `
      ...
      <button type="button" id="bulkDeleteBtn" class="btn bulk-delete-btn" disabled>一鍵刪除已選訂單</button>`;
  ...
  async function singleDelete(btn) {
    ...
    const orderNo = row?.querySelector('td:nth-child(3) b')?.textContent?.trim() || '此訂單';
    if (!confirm(`確定永久刪除 ${orderNo}？\n商品明細與廠商回覆會一起刪除，LINE 匯入歷史會保留。`)) return;
    btn.disabled = true;
    const old = btn.textContent;
    btn.textContent = '刪除中…';
    try {
      const d = await deleteViaAdminApi([orderId]);
      alert(`刪除完成：${d.deleted_count} 筆`);
      $('reloadBtn')?.click();
    } catch (e) {
      alert(`刪除失敗：${e.message}`);
      btn.disabled = false;
      btn.textContent = old || '刪除';
    }
  }
  ...
  async function bulkDeleteSelected() {
    ...
    const btn = $('bulkDeleteBtn');
    btn.disabled = true;
    btn.textContent = '批次刪除中…';
    try {
      const d = await deleteViaAdminApi(ids);
      alert(`批次刪除完成：成功刪除 ${d.deleted_count} 筆。`);
      $('reloadBtn')?.click();
    } catch (e) {
      alert(`批次刪除失敗：${e.message}`);
      btn.disabled = false;
      btn.textContent = '一鍵刪除已選訂單';
    }
  }
